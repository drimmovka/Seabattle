...
void Game::signObserver(GameObserver& observer) {
    observer.bind(&_should_be_displayed);
}
...
bool Game::roundWon() {
    bool res = _game_data->enemy_ships_manager->allDestroyed();
    if (res) {
        _should_be_displayed = true;
    }
    return res;
}
...
void Game::setSettings(int game_map_width, int game_map_height, std::vector<int>& ship_sizes) {
    ...
    _should_be_displayed = true;                        // allow the game to be displayed
    // will display user empty field and enemy field
}

void Game::userPlaceShipsRandomly() {
    this->placeShipsRandomly(_game_data->user_ships_manager, _game_data->user_game_map);
    _should_be_displayed = true;                        // allow the game to be displayed
    // will display fields with user ships and enemy field with enemy ships
}
